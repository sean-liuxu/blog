...
```
Pod 申请: nvidia.com/gpu=1, nvidia.com/gpumem=3000, nvidia.com/gpucores=30
  │
  ↓ Scheduler 找到有可用 vGPU 的节点
  ↓ Webhook 注入环境变量:
  │   CUDA_DEVICE_MEMORY_LIMIT=3000
  │   CUDA_DEVICE_SM_LIMIT=30
  │   LD_PRELOAD=/k8s-vgpu/lib/nvidia/libvgpu.so
  │
  ↓ Device Plugin 分配 vGPU，挂载 libvgpu.so 到容器
  │
  ↓ 容器内 CUDA 程序调用 cudaMalloc(4000MB)
  │   → libvgpu.so 拦截
  │   → "4000 > limit 3000, 返回 OOM"
  │
  ↓ nvidia-smi 也只显示 3000MB（伪造输出）
```
